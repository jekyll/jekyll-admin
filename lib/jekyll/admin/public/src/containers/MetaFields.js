...
export class MetaFields extends Component {
  ...
  render() {
    const { fields, metadata, addField, removeField, updateFieldKey,
      updateFieldValue, moveArrayItem, convertField, key_prefix } = this.props;

    const { id, raw_content, content, previous, next, name, path, url,
    dir, relative_path, collection, excerpt, output, date, categories, slug, ext, title, tags, ...rest } = metadata;

    let metafields = _.map(rest, (field, key) => {
      let type = "simple";
