...
export class Documents extends Component {
  ...
  renderRows() {
    ...
    return _.map(currentDocuments, (doc) => {
      let { id, title, url, ext, collection } = doc;
      // TODO fix this when react-router allows slashes in params
      // replace all '/' with '-' except the first one
      let dashedId = id.replace(/(?!^)\//g, '-');
      let to = collection == "posts" ? `${ADMIN_PREFIX}/collections/posts${dashedId+ext}`:
        `${ADMIN_PREFIX}/collections${id+ext}`;
      return (
        <tr key={id}>
          <td className="row-title">
            ...
            <div className="row-actions">
              <a onClick={(id) => this.handleClickDelete(id)}>Delete</a>
