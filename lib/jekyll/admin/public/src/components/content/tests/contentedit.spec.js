...
describe('Components::ContentEdit', () => {
  it('should render correctly', () => {
    const { component, breadcrumbs, props } = setup();
    expect(
      breadcrumbs.prop('breadcrumbs')[0].link
    ).toBe(`/${props.contentType}/${doc.collection_name}`);
    expect(
      breadcrumbs.prop('breadcrumbs')[0].text
    ).toBe(doc.collection_name);
    expect(
      breadcrumbs.prop('breadcrumbs')[1].text
    ).toBe(doc.meta.path);
  });
