...
const sampleRows = [
  ...
  ['testing-posts2', 'Testing Posts 2', 'test2', 'test2, post', 'published']
];
...
function setup(rows = sampleRows) {
  ...
  let component = mount(
    <ContentTable
      ...
      rows={rows}
      onClickDelete={actions.deletePost} />
  );
...
describe('Components::ContentTable', () => {
  ...
  it('should render correctly', () => {
    ...
    let actual = '/' + compProps.contentType + '/' + compProps.rows[0][0];
  ...
  it('should not dispach deletePost when delete butten clicked', () => {
    ...
    expect(component.props().onClickDelete.calls.length).toBe(0); //prompt?
  });
});
