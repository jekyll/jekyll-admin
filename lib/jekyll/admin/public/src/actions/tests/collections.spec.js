...
describe('Actions::Collections', () => {
  ...
  it('fetches the document', () => {
    ...
    let id = "the-revenant";
    nock(API)
      .get(`/collections/${collection_name}/documents/${id}`)
      .reply(200, doc);
  ...
  it('deletes the document', () => {
    ...
    nock(API)
      .delete(`/collections/${collection_name}/documents/${id}`)
  ...
  it('updates the document', () => {
    ...
    let id = "the-revenant";
    nock(API)
      .put(`/collections/${collection_name}/documents/${id}`)
      .reply(200, { ...doc });
    ...
    return store.dispatch(actions.putDocument(doc.collection, id))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });
  ...
  it('creates PUT_DOCUMENT_FAILURE when updating document failed', () => {
    ...
    return store.dispatch(actions.putDocument("movies", id))
      .then(() => {
  ...
  it('creates VALIDATION_ERROR if required field is not provided.', () => {
    ...
    store.dispatch(actions.putDocument(id, "movies"));
    expect(store.getActions()).toEqual(expectedActions);
  });
