...
describe('Actions::Collections', () => {
  ...
  it('updates the document', () => {
    ...
    const expectedActions = [
      ...
      { type: types.UPDATE_DOCUMENT_SUCCESS, doc }
    ];

    const store = mockStore({metadata: doc});

    return store.dispatch(actions.putDocument(id))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });

  it('creates UPDATE_DOCUMENT_FAILURE when updating document failed', () => {
    let id = "the-revenant";
    nock(API)
      .put('/collections/' + id)
      .replyWithError('something awful happened');
    ...
    const expectedActions = [
      { type: types.VALIDATED },
      { type: types.UPDATE_DOCUMENT_FAILURE, error: 'something awful happened' }
    ];
  ...
  it('creates VALIDATION_ERROR if required field is not provided.', () => {
    ...
    const store = mockStore({metadata: { body: '',title: ''} });

    store.dispatch(actions.putDocument(id));
    expect(store.getActions()).toEqual(expectedActions);
  });
