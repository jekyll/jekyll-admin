...
describe('Actions::Pages', () => {
  ...
  it('updates the page', () => {
    ...
    const expectedActions = [
      ...
      { type: types.UPDATE_PAGE_SUCCESS, page }
    ];

    const store = mockStore({metadata: page});

    return store.dispatch(actions.putPage(id))
      .then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
  });

  it('creates UPDATE_PAGE_FAILURE when updating page failed', () => {
    let id = "about";
    nock(API)
      .put('/pages/' + id)
      .replyWithError('something awful happened');
    ...
    const expectedActions = [
      { type: types.VALIDATED },
      { type: types.UPDATE_PAGE_FAILURE, error: 'something awful happened' }
    ];
  ...
  it('creates VALIDATION_ERROR if required field is not provided.', () => {
    ...
    store.dispatch(actions.putPage(id));
    expect(store.getActions()).toEqual(expectedActions);
  });
