...
export function putDocument() {
  return (dispatch, getState) => {
    ...
    const { path, collection, id, raw_content } = metadata;
    ...
    const doc = JSON.stringify({
      document_id: id,
      meta: _.omit(metadata, [
        'id', 'raw_content', 'content', 'output', 'slug', 'relative_path', 'previous',
        'next', 'ext', 'excerpt', 'collection', 'url', 'title'
      ]), // TODO
      body: raw_content
    });
    // TODO dispatch({type: ActionTypes.PUT_DOCUMENT_REQUEST, doc});
    const dashedID = path.substring(path.lastIndexOf('/') + 1);
    return put(
      putCollectionDocumentUrl(collection, dashedID),
      doc,
