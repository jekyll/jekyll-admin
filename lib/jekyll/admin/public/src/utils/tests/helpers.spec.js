...
import { state, emptyState, form } from './fixtures';
...
describe('Form helper functions:', () => {
  describe('validateForm', () => {
    it('should generate error messages if not validated', () => {
      let values = {title: '', draft: false, path: 'awesome.ph'};
      let validators = {
        title: 'required',
        draft: 'boolean'
      };
      let errors = {
        ...
        draft: 'The draft field must be boolean type.'
      };
      let errorMessages = validateForm(values, validators, errors);
      expect(errorMessages).toInclude('The title field is required.');
      values.title = "GSoC";
      values.draft = "GSoC";
      errorMessages = validateForm(values, validators, errors);
      expect(errorMessages).toEqual(['The draft field must be boolean type.']);
    });
    it('should generate error message if validator does not exist', () => {
      let values = {title: '', body: 'Some body text.', path: 'awesome.ph'};
      let validators = {title: 'bad_validator'};
      let errors = {title: 'Bad validator.'};
      let errorMessages = validateForm(values, validators, errors);
      expect(errorMessages).toInclude('Bad validator.');
    });
  });
});
